...
RTPSink::Source::Source(uint16_t seq, const sp<ABuffer> &buffer,
		const sp<AMessage> queueBufferMsg) :
		mQueueBufferMsg(queueBufferMsg), mProbation(kMinSequential) {
	initSeq(seq);
	mMaxSeq = seq - 1;
...
void RTPSink::Source::addReportBlock(uint32_t ssrc, const sp<ABuffer> &buf) {
...
RTPSink::RTPSink(const sp<AXNetworkSession> &netSession,
		const sp<IGraphicBufferProducer> &surfaceTex) :
		mNetSession(netSession), mSurfaceTex(surfaceTex), mRTPPort(0), mRTPSessionID(
				0), mRTCPSessionID(0), mFirstArrivalTimeUs(-1ll), mNumPacketsReceived(
				0ll), mRegression(1000), mMaxDelayMs(-1ll),
...
status_t RTPSink::init(bool useTCPInterleaving) {
    ...
    for (clientRtp = 15550;; clientRtp += 2) {
        int32_t rtpSession;
		status_t err = mNetSession->createUDPSession(clientRtp, rtpNotify,
				&rtpSession);
		...
		int32_t rtcpSession;
		err = mNetSession->createUDPSession(clientRtp + 1, rtcpNotify,
				&rtcpSession);
...
void RTPSink::onMessageReceived(const sp<AMessage> &msg) {
	...
	case kWhatRTCPNotify: {
		...
		case AXNetworkSession::kWhatDatagram: {
			int32_t sessionID;
			CHECK(msg->findInt32("sessionID", &sessionID));
...
status_t RTPSink::parseRTP(const sp<ABuffer> &buffer) {
	...
	ALOGW(" the arrivalTime US:  %lld", arrivalTimeUs);
	...
	ALOGW("seqNo: %d, SSRC 0x%08x, diff %lld", seqNo, srcId,
			rtpTime - arrivalTimeMedia);
	...
	if (mRegression.approxLine(&n1, &n2, &b)) {
		ALOGV("Line %lld: %.2f %.2f %.2f, slope %.2f", mNumPacketsReceived, n1,
				n2, b, -n1 / n2);
	...
	if (index < 0) {
		...
		sp<AMessage> queueBufferMsg = new AMessage(
				TunnelRenderer::kWhatQueueBuffer, mRenderer->id());
...
status_t RTPSink::parseRTCP(const sp<ABuffer> &buffer) {
	...
	while (size > 0) {
		...
		default: {
			ALOGW("Unknown RTCP packet type %u of size %d", (unsigned) data[1],
					headerLength);
			break;
		}
		}
...
status_t RTPSink::parseSR(const uint8_t *data, size_t size) {
	...
	ALOGV("SR: ssrc 0x%08x, ntpTime 0x%016llx, rtpTime 0x%08x", id, ntpTime,
			rtpTime);

	return OK;
}

status_t RTPSink::connect(const char *host, int32_t remoteRtpPort,
		int32_t remoteRtcpPort) {
    ALOGI("connecting RTP/RTCP sockets to %s:{%d,%d}",
          host, remoteRtpPort, remoteRtcpPort);
    mIsConnectRemotePort = true;

	status_t err = mNetSession->connectUDPSession(mRTPSessionID, host,
			remoteRtpPort);
	if (err != OK) {
	...
	ALOGI("=====OK=====connecting RTP sockets to %s:%d", host, remoteRtpPort);
	...
	ALOGI("=====OK=====connecting RTCP sockets to %s:%d", host, remoteRtcpPort);
	scheduleSendRR();
