...
struct AXNetworkSession::NetworkThread : public Thread {
    NetworkThread(AXNetworkSession *session);

protected:
    virtual ~NetworkThread();

private:
    AXNetworkSession *mSession;
...
struct AXNetworkSession::Session : public RefBase {
    enum State {
        CONNECTING,
        CONNECTED,
        LISTENING_RTSP,
...
AXNetworkSession::NetworkThread::NetworkThread(AXNetworkSession *session)
    : mSession(session) {
}

AXNetworkSession::NetworkThread::~NetworkThread() {
}

bool AXNetworkSession::NetworkThread::threadLoop() {
    mSession->threadLoop();
...
AXNetworkSession::Session::Session(
        int32_t sessionID,
        State state,
        int s,
        const sp<AMessage> &notify)
...
int32_t AXNetworkSession::Session::sessionID() const {
    return mSessionID;
}

int AXNetworkSession::Session::socket() const {
    return mSocket;
}

void AXNetworkSession::Session::setIsRTSPConnection(bool yesno) {
    mIsRTSPConnection = yesno;
}

sp<AMessage> AXNetworkSession::Session::getNotificationMessage() const {
    return mNotify;
}

bool AXNetworkSession::Session::isRTSPServer() const {
    return mState == LISTENING_RTSP;
}

bool AXNetworkSession::Session::isTCPDatagramServer() const {
    return mState == LISTENING_TCP_DGRAMS;
}

bool AXNetworkSession::Session::wantsToRead() {
    return !mSawReceiveFailure && mState != CONNECTING;
}

bool AXNetworkSession::Session::wantsToWrite() {
...
status_t AXNetworkSession::Session::readMore() {
    if (mState == DATAGRAM) {
        status_t err;
        do {
...
status_t AXNetworkSession::Session::writeMore() {
    if (mState == DATAGRAM) {
...
status_t AXNetworkSession::Session::sendRequest(const void *data, ssize_t size) {
...
void AXNetworkSession::Session::notifyError(
...
void AXNetworkSession::Session::notify(NotificationReason reason) {
    sp<AMessage> msg = mNotify->dup();
    msg->setInt32("sessionID", mSessionID);
...
AXNetworkSession::AXNetworkSession()
    : mNextSessionID(1) {
...
status_t AXNetworkSession::start() {
    if (mThread != NULL) {
    ...
    status_t err = mThread->run("AXNetworkSession", ANDROID_PRIORITY_AUDIO);
...
status_t AXNetworkSession::stop() {
    if (mThread == NULL) {
        return INVALID_OPERATION;
    }
...
status_t AXNetworkSession::createRTSPClient(
...
status_t AXNetworkSession::createRTSPServer(
...
status_t AXNetworkSession::createUDPSession(
...
status_t AXNetworkSession::createTCPDatagramSession(
        const struct in_addr &addr, unsigned port,
        const sp<AMessage> &notify, int32_t *sessionID) {
    return createClientOrServer(
...
status_t AXNetworkSession::destroySession(int32_t sessionID) {
...
// static
status_t AXNetworkSession::MakeSocketNonBlocking(int s) {
...
status_t AXNetworkSession::createClientOrServer(
        Mode mode,
...
status_t AXNetworkSession::connectUDPSession(
...
status_t AXNetworkSession::sendRequest(
...
void AXNetworkSession::interrupt() {
...
void AXNetworkSession::threadLoop() {
    fd_set rs, ws;
    FD_ZERO(&rs);
    FD_ZERO(&ws);
